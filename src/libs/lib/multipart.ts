...
function getValueIgnoringKeyCase(object, key): string {
  const foundKey = Object.keys(object)
    .find((currentKey) => currentKey.toLocaleLowerCase() === key.toLowerCase()) as string;
...
export default (event): any => {
  ...
  return data.split(boundary)
    ...
    .map((item) => {
      ...
      const header = tmp[0];
      let content: string | Buffer = tmp[1];
      let name;
      if (deviceType === '0') { // mobile app
