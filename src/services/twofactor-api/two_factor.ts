...
import { genericTwofactorProvider, TwofactorData } from './providers/generic-twofactor-provider';
...
// /two-factor GET
export const getHandler = async (event, context, callback): Promise<void> => {
...
export const disableTwofactorHandler = async (event, context, callback): Promise<void> => {
...
export const getRecoveryCode = async (event, context, callback): Promise<void> => {
...
export const recover = async (event, context, callback): Promise<void> => {
  console.log('2FA get recovery code triggered', JSON.stringify(event, null, 2));
...
export const getAuthenticatorHandler = async (event, context, callback): Promise<void> => {
  ...
  try {
    const twofactor: Twofactor<string> = authenticatorProvider
      .getTwofactor(user, TwoFactorType.Authenticator);
    const result: GetAuthenticatorResponse = authenticatorProvider
      .getAuthenticatorResponse(twofactor);
...
export const activateAuthenticatorHandler = async (event, context, callback): Promise<void> => {
  console.log('2FA get authenticator handler triggered', JSON.stringify(event, null, 2));
  ...
  try {
    ...
    if (verified) {
      ...
      const result: GetAuthenticatorResponse = {
...
export const getU2f = async (event, context, callback): Promise<void> => {
...
export const generateU2fChallenge = async (event, context, callback): Promise<void> => {
  console.log('2FA get u2f handler triggered', JSON.stringify(event, null, 2));
  ...
  try {
    const challengeResponse: ChallengeResponse = await u2fProvider
      .createU2fChallenge(user, TwoFactorType.U2fRegisterChallenge);
...
// /two-factor/u2f POST
export const activateU2f = async (event, context, callback): Promise<void> => {
  console.log('2FA get u2f handler triggered', JSON.stringify(event, null, 2));
  ...
  try {
    const twofactor: Twofactor<ChallengeResponse> = u2fProvider
      .getTwofactor(user, TwoFactorType.U2fRegisterChallenge);
  ...
  try {
    const registration: RegistrationCheckResponse = u2fProvider
      .checkRegistration(challenge, deviceResponse);
    ...
    if (registration.successful) {
      ...
      const twofactor: Twofactor<U2FRegistration> = u2fProvider
        .getTwofactor(user, TwoFactorType.U2f);
...
export const deleteU2f = async (event, context, callback): Promise<void> => {
  ...
  try {
    const index: number = registrations.findIndex((registration) => registration.id === body.id);
