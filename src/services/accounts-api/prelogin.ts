...
export class PreloginLambda extends BaseLambda {
  public handler = async (event: APIGatewayEvent): Promise<APIGatewayProxyResult> => {
    ...
    const bodyEither: E.Either<Error, RequestBody> = this.parseRequestBody(event.body);
    ...
    return userTaskEither().then(
      (userEither: E.Either<Error, Item>) => pipe(
        userEither,
        E.fold(
          (error: Error) => this.validationError(`Error: ${error.message}`),
